...
|   -> AS400 Core: Agent/Source Code Created & Active Temporary Status Set                          |
...
| -> 30-Day SLA Countdown Started for Physical Contract Delivery                                    |
+----------------------------------------------------------------------------------------------------+
                                                  |
                                                  v
+----------------------------------------------------------------------------------------------------+
...
|   - If SLA Breached (>30d) -> Auto-Suspension Daemon sets "SUSPENDED_TEMPORARY" in AS400 & PCSDIS  |
...
2. **Execution Flow**:
   - `Step 1`: Call `IAs400ApiClient.CreateAgentCodeAsync` $\rightarrow$ generate `AgentCode` & `SourceCode`.
   - `Step 2`: Call `IAparApiClient.ConfigureLedgerAccountAsync` with `AgentCode`.
   - `Step 3`: Call `ISapApiClient.CreateBusinessPartnerAsync` with Tax ID and Bank Account.
   - `Step 4`: Call `IPcsdisApiClient.SetupNodeAndUeAsync` with branch node, assigned UE, and commission rate.
3. **Idempotency & Partial Failure Handling**:
   ...
   - Once all 4 subsystems report `SUCCESS`, the agent is promoted to `ACTIVE_TEMPORARY` (Provisional Selling Active).
